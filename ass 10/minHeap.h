...
template<class myType>
void minHeap<myType>::insert(const myType vertex, const int dist) {
	if (++count == heapSize)
		resize();
    myHeap[count].vertexNum = vertex;
    myHeap[count].distance = dist;
	reheapUp(count);
}
...
myType minHeap<myType>::deleteMin() {
	...
	myType vertex = myHeap[1].vertexNum;
...
void minHeap<myType>::reheapUp(unsigned int position) {
	...
	if (myHeap[position].distance < myHeap[parentNode].distance) {
		node temp = myHeap[position];
...
void minHeap<myType>::reheapDown(unsigned int position) {
	...
	if (right > count)
		...
		if (myHeap[left].distance <= myHeap[right].distance)
			minPos = left;
		else minPos = right;
	}
	//time to swap baby
	if (myHeap[minPos].distance < myHeap[position].distance) {
		temp = myHeap[minPos];
...
void minHeap<myType>::resize() {
	heapSize = heapSize * 2;
	node * newHeap = new node[heapSize];
